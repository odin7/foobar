...
def answer(str):
    out = ''
    sig_out = deque()  # use dequeu instead of list to efficientrly insert(0) and pop(0)
    for symb in str:
        if ord('0') <= ord(symb) <= ord('9'):  # digits
            out += symb
        else:   # operators
            if not sig_out or ord(sig_out[0]) >= ord(symb):  # ASCII * < +
                sig_out.insert(0, symb)
                continue
            while sig_out and ord(sig_out[0]) < ord(symb):
                out += sig_out.popleft()
            sig_out += symb
    return out + ''.join(sig_out)
