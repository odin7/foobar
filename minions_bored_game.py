...
def answer(t, n):
    # board[i] holds the number of ways to get to square i
    # board_prev is the board state for the previous dice roll
    board, board_prev = [0] * n, []
    board[0] = 1
    for _ in repeat(None, t):  # calculate for t dice throws
        board, board_prev = board_prev, board
        board = [(board_prev[i - 1] if i > 0 else 0) + board_prev[i] + (board_prev[i + 1] if i + 1 < n - 1 else 0) for i in xrange(n)]

    return board[- 1] % 123454321
